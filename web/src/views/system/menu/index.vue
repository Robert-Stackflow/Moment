              ...
              withDirectives(
                h(
                  NButton,
                  {
                    size: 'tiny',
                    type: 'error',
                    style: `display: ${row.children && row.children.length > 0 ? 'none' : ''};margin-left:5px;`, //有子菜单不允许删除
                  },
                  {
                    default: () => '删除',
  ...
  <CommonPage show-footer :title="$t('views.setting.label_menu_list')">
    <template #action>
      <NButton v-permission="'post/api/v1/menu/create'" type="primary" @click="handleClickAdd">
        <TheIcon icon="material-symbols:add" :size="18" class="mr-5" />{{ $t('views.setting.label_new_root_menu') }}
      </NButton>
    </template>
